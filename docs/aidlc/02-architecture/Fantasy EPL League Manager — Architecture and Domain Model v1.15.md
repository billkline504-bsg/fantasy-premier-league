...
## Architecture and Domain Model — Version 1.15

**Document Status:** Baseline Architecture
**Version:** 1.15
**Inputs:**
- Business Requirements Document v1.17 (`Fantasy EPL League Manager — Business Requirements Document v1.17.md`) — authoritative business rules (BR-001…BR-339)
...
This document does not restate business rules; it cites them by ID (`BR-###`) and adds the structural decisions needed to implement them. Where the BRD (v1.8) and the earlier Domain Specification (v1.0) differ, **the BRD is authoritative** (it is the later, more detailed baseline — see BRD §60, "Final Baseline"). Section 2 reconciles the two.
...
| Player & EPL Data | `EplFantasy.PlayerData` | Player, Club, Fixture, Gameweek (reference data; see ADR-009); ClubStanding (read-model, BR-329–BR-335) |
...
### UsernameHistory (Entity, append-only — BR-326, BRD v1.13)

```text
UsernameHistory
---------------
UsernameHistoryId : Guid
UserId             : Guid (FK)
Username           : string
EffectiveFrom      : DateTimeOffset
EffectiveTo        : DateTimeOffset?   (null = currently active)
```

Every `UsernameChanged` event handler closes the User's currently-active `UsernameHistory` row (`EffectiveTo = now`) and opens a new one (`EffectiveFrom = now`, `EffectiveTo = null`) in the same transaction — including the very first row, opened at `UserRegistered` time with the initial username. This is what makes BR-326 possible without adding a username field to every historical table (`GameweekRoster`, `DraftSelection`, `HeadToHeadMatch`, `LeagueStanding`, etc.): a historical read resolves the display username by finding the `UsernameHistory` row for that record's `UserId` where `EffectiveFrom <= record.CreatedAt` and (`EffectiveTo IS NULL` or `record.CreatedAt < EffectiveTo`), rather than by joining straight to `User.Username`. This is a read-time resolution, not a value copied onto every row, so it never needs backfilling into existing historical data beyond seeding one open-ended `UsernameHistory` row per existing `User` at migration time.
...
`UserProfile` no longer carries a `NotificationPreferencesSummary` field (present through Version 1.14): BR-338 (BRD v1.17) establishes that notification preferences are held per `LeagueMembership`, not once per `User`, so there is no single global summary value for this aggregate to own. A screen wanting to show all of a User's notification settings at once (e.g., the Profile screen) reads across that User's `LeagueMembership`-keyed `NotificationPreference` rows (§6.9) directly, rather than through a rollup field here.
...
GameweekReminderLeadTime          : TimeSpan = 24 hours      (BR-152, BR-309, BRD v1.8)
...
**Squad View query (BR-314–BR-320, BRD v1.10):** the read endpoint backing F-007.5's Squad View (Section 9.2) composes `SquadPlayer` (this section — `AcquisitionType` for BR-318, `ReplacementEligibleAt IS NOT NULL` for BR-320) with `PlayerSeasonStatistics` (Section 6.6, for BR-317) and a membership check against the FantasyTeam's current-Gameweek `GameweekRoster`/`RosterPlayer` (Section 6.5, for BR-319's roster indicator). It accepts the same `position`, `search`, and `sort` query parameters as the Draft Player Pool endpoint (Section 6.4), reusing the identical filter/search/sort implementation rather than a second one — the two read-models differ only in which rows they draw from (a Draft's remaining player pool vs. one FantasyTeam's owned squad) and which extra columns they project (ownership status vs. acquisition type/roster membership/replacement eligibility).
...
**Skip/re-queue visibility (BR-323–BR-325, BRD v1.12):** every time the ADR-012 sweep skips a pick — whether its first skip or a makeup pick timing out again — it raises a `DraftPickSkipped` domain event (`IsRequeue: bool` distinguishing an initial skip from a re-queue) in addition to mutating `PendingMakeupPicks`. This event is what a Reporting-context projection consumes to render the skip/re-queue history a Draft UX screen needs (BR-323, BR-325) — without it, a skip would only be inferable after the fact from a gap in `DraftSelection.PickNumber`, which is not a practical way to drive a UI. BR-324's pending-queue display, in contrast, needs no event at all: it reads `Draft.PendingMakeupPicks` directly, since that field already holds the queue's current, live order.
...
**Draft Player Pool query (BR-310–BR-312, BRD v1.9):** the read endpoint backing F-005.5's player pool (Section 9.2) accepts `position`, `search`, and `sort` query parameters — a case-insensitive substring match on player name for `search`, and any displayed column (name, club, position, or the `PlayerSeasonStatistics` fields in Section 6.6) for `sort` — rather than exposing separate endpoints per filter/sort combination. This applies identically to the Initial Draft and the Secondary/Replacement Draft, since both reuse the same shared draft engine and player-pool read-model.

Domain events: `DraftStarted`, `DraftTimerExtended` (BR-058), `PlayerDrafted`, `DraftPickSkipped` (BR-282, BR-323/BR-325, BRD v1.12), `DraftCompleted`.
...
### Weekly Roster Screen — Gameweek Schedule & Opponent Display (BR-336–BR-337, BRD v1.16)

Neither rule adds a persisted entity. Both are read-time projections computed when the roster screen is loaded, using data that already exists by this point in the build sequence (`Fixture`/`Gameweek`, Player & EPL Data context, §6.10):

- **BR-336** (Gameweek EPL schedule): the same `Fixture` rows already returned by `GET /api/v1/epl/gameweeks/{gameweekId}/fixtures` (§9.2, F-004.6) are rendered on the roster screen — no new query shape, just a second surface for one already-built read.
- **BR-337** (per-player opponent): for each `RosterPlayer`/`SquadPlayer` shown, the application resolves `Player.CurrentClubId`, finds the `Fixture` row for the roster's `GameweekId` where that club is either `HomeClubId` or `AwayClubId`, and displays the other club as the opponent, tagged Home or Away. A player whose club has no fixture that Gameweek (a postponed fixture, BR-100–BR-103) shows no opponent rather than a stale or guessed one.

This computation is naturally placed in the Roster Management context's application service backing the roster-screen read endpoint, since it depends on both `SquadPlayer`/`RosterPlayer` (this context) and `Fixture` (Player & EPL Data context) — a read-only cross-context join, not a write path, so it requires no new domain event and no change to `GameweekRoster`'s state machine.
...
GameweekId          : Guid (FK)
PlayerId            : Guid (FK)
MinutesPlayed        : int      (BR-313, BRD v1.9 — added for player-pool statistics display)
FantasyPoints        : int
Goals                : int
GoalsConceded        : int
OwnGoals             : int
...
### PlayerSeasonStatistics (Read-model projection, Reporting context — BR-313, BRD v1.9)
...
SeasonId       : Guid (FK)
PlayerId       : Guid (FK)
MinutesPlayed  : int   (sum of PlayerPerformance.MinutesPlayed across all Gameweeks-to-date this Season)
GamesPlayed    : int   (count of Gameweeks-to-date this Season where PlayerPerformance.MinutesPlayed > 0)
FantasyPoints  : int   (sum of PlayerPerformance.FantasyPoints across all Gameweeks-to-date this Season)
AsOfGameweekId : Guid (FK)
```

This is a read-only projection over `PlayerPerformance`, not a separately persisted aggregate with its own invariants — it is recomputed (or incrementally maintained) as each Gameweek's official data is ingested (Section 10), consistent with the Reporting context's role as read-model projections only (Section 5). It backs the Draft Player Pool statistics display (F-005.5, BR-313): before a Season's first Gameweek has been scored — i.e., throughout the Initial Draft — no `PlayerPerformance` rows yet exist for that Season, so all three figures resolve to zero for every player rather than being omitted, keeping the player pool's column layout identical across Draft types.
...
**Audit log filtering and detail expansion (BR-321–BR-322, BRD v1.11):** the read endpoint backing F-011.1 (Section 9.2) accepts `actionType`, `from`, `to`, and `fantasyTeamId` query parameters. `TargetEntityType`/`TargetEntityId` are generic (not every `AdministrativeAction` targets a `FantasyTeam` — a `ConfigurationChanged` entry targets the `League` or `Season` itself), so a row with no resolvable `FantasyTeamId` is surfaced under a "League Settings" pseudo-value for the `fantasyTeamId` filter rather than being unfilterable or mis-attributed to a team. BR-322's per-entry detail expansion requires no separate endpoint or additional query: `BeforeState`/`AfterState` are already part of every row this endpoint returns, so expansion is purely a client-side reveal of data already in hand.

### SecurityEvent (Read-model / lightweight log entry — BR-327, BRD v1.14)

```text
SecurityEvent
-------------
SecurityEventId : Guid
EventType       : SecurityEventType (RateLimitBlocked | ...)
Endpoint        : string
Scope           : string   (e.g., "IP 203.0.113.44 + username davecoach")
Detail          : string
OccurredAt      : DateTimeOffset
```

This is a platform-level log, unrelated to any single `LeagueId` — unlike `AdministrativeAction` (which is always League-scoped), `SecurityEvent` rows are written by the rate-limiting middleware itself whenever a configured limit blocks a request, satisfying BR-170's existing requirement that security-relevant events be logged, but as a queryable table rather than only an application-log line. This is what BR-327's System-Administrator-only read endpoint (Section 9.2) queries; it requires no change to `AdministrativeAction`, since a rate-limit block is not an administrator-privileged mutation and has no `LeagueId` to attach to.

**CSRF posture (BR-328, BRD v1.14):** unlike `SecurityEvent`, this is not backed by a persisted entity — it is a read of application configuration (whether cookie-based authentication is enabled per ADR-006, and, if so, that its CSRF middleware is active), exposed through the same System-Administrator-only endpoint group.

## 6.9 Notifications Context

### NotificationPreference (League-scoped — BR-338, BRD v1.17)

```text
NotificationPreference
-----------------------
LeagueMembershipId : Guid (FK)   (BR-338 — one User's participation in one League, not the User alone)
EventType    : NotificationEventType (GameweekReminder | WeeklyScore | WeeklyStandings)
Channel      : NotificationChannel (Email | Sms)
Enabled      : bool
```

Unique constraint on `(LeagueMembershipId, EventType, Channel)`. Because `GameweekReminder`, `WeeklyScore`, and `WeeklyStandings` are all generated within the context of one specific League (a roster-lock deadline, a Gameweek score, a standings position all belong to exactly one League a User plays in), keying this row by `LeagueMembershipId` rather than `UserId` means a User with three League Memberships holds up to three independent rows per event type/channel pair — never one shared row silently governing every League at once (BR-338). This also means a `NotificationPreference` row's lifecycle now follows its owning `LeagueMembership`: it is seeded (all channels disabled by default) when the `LeagueMembership` is created, and is no longer queried once that `LeagueMembership`'s status is `left`.

```text
NotificationRequest   (outbox entry — Section 12.2)
-------------------
RequestId          : Guid
UserId              : Guid (FK)             (delivery target — a User has one email/phone regardless of League)
LeagueMembershipId  : Guid (FK)             (BR-338 — which League's NotificationPreference row gates this request)
...
`NotificationRequest` keeps `UserId` because delivery (which inbox, which phone number) is still a User-level concern, but adds `LeagueMembershipId` so the outbox worker checks the correct League's `NotificationPreference` row (BR-226) rather than an ambiguous or global one — a request for League A's Weekly Score never reads League B's preference row, even though both belong to the same User.

## 6.10 Player & EPL Data Context — EPL Table & Fixtures Display (Read-models — BR-329–BR-335, BRD v1.15)

`Player`, `Club`, `Fixture`, and `Gameweek` are the reference-data aggregate roots already named in the module map (§5) and populated by F-004.1/F-004.2 (BR-227–BR-230); this Architecture document had not previously detailed their shape, since prior BRD versions used them only as inputs to Fantasy scoring/scheduling. BR-329–BR-335 are the first requirement to expose them directly to users as their own page, so a small read-model is added to support it:

```text
ClubStanding   (Read-model, recomputed — not user-editable, same pattern as LeagueStanding in §6.7)
------------
SeasonId       : Guid (FK)
ClubId         : Guid (FK)
Position       : int
...
GoalDifference : int   (computed)
Points         : int
```

`ClubStanding` is recomputed from `Fixture` results as official data is synced (F-004.1/F-004.2/F-004.3), the same "official data drives a recalculated projection" shape already used for `LeagueStanding` (§6.7) and `PlayerSeasonStatistics` (§6.6) — it is never written to directly by any user action, and carries no Administrator-override path (BR-334): a `ScoreOverride` corrects Fantasy scoring, never the real-world EPL table. One `ClubStanding` row set exists per `SeasonId`, so a past Season's final table (BR-331) is simply that Season's already-final row set, retained under the same historical-retention policy as other competitive read-models (§8.2). The Gameweek Fixtures page (BR-332–BR-333) requires no new entity: it queries existing `Fixture` rows filtered by `GameweekId`, already keyed for this by F-004.2.
...
| Draft | `DraftStarted`, `DraftTimerExtended`, `PlayerDrafted`, `DraftPickSkipped`, `DraftCompleted` |
...
| `username_history` | `username_history_id` | Partial unique index `(user_id) WHERE effective_to IS NULL` (at most one open/active row per User, BR-326) |
...
| Fantasy Team | `GET /api/v1/leagues/{leagueId}/seasons/{seasonId}/fantasy-teams/{fantasyTeamId}/squad?position=&search=&sort=` (BR-314–BR-320) |
| Draft | `POST /api/v1/drafts/{draftId}/start`, `POST /api/v1/drafts/{draftId}/picks`, `POST /api/v1/drafts/{draftId}/timer/extend`, `GET /api/v1/drafts/{draftId}/player-pool?position=&search=&sort=` (BR-310–BR-313) |
| Roster | `GET /api/v1/fantasy-teams/{fantasyTeamId}/gameweeks/{gameweekId}/roster` (includes each player's Gameweek opponent/home-away and the Gameweek's full fixture list — BR-336–BR-337), `PUT .../roster`, `PUT .../roster/captain` |
...
| Administration | `GET /api/v1/leagues/{leagueId}/audit?actionType=&from=&to=&fantasyTeamId=` (BR-321–BR-322) |
| Administration (System Administrator, platform-level — not League-scoped) | `GET /api/v1/admin/security/rate-limits`, `GET /api/v1/admin/security/events` (BR-327), `GET /api/v1/admin/security/csrf-status` (BR-328) |
| Player & EPL Data (platform-level, not League-scoped) | `GET /api/v1/epl/seasons/{eplSeasonId}/table` (BR-329–BR-331), `GET /api/v1/epl/gameweeks/{gameweekId}/fixtures` (BR-332–BR-333) |
...
Incorporates BRD v1.8's resolution of the one item raised while writing the Phase 4 feature specifications (BR-309): adds `GameweekReminderLeadTime` (default 24 hours) to `LeagueConfiguration`/`SeasonConfiguration` (§6.2), following the same default/override/audit/locking model as every other configurable parameter (ADR-011).

## Version 1.7

Incorporates BRD v1.9's four Draft Player Pool UX requirements (BR-310–BR-313), identified while reviewing an illustrative HTML mock-up of the Draft Board (F-005.5): adds `MinutesPlayed` to `PlayerPerformance` (§6.6); introduces the `PlayerSeasonStatistics` read-model projection (§6.6) aggregating season-to-date Minutes Played, Games Played, and Official FPL Points per Player, sourced from `PlayerPerformance` and resolving to zero before a Season's first Gameweek is scored; and documents player-pool position filtering, name search, and column sorting as query parameters on the existing Draft Player Pool read endpoint (§6.4, §9.2) rather than as new endpoints.

## Version 1.8

Incorporates BRD v1.10's formalization of the Squad View (BR-314–BR-320), identified while reviewing an illustrative HTML mock-up of F-007.5: documents the Squad View read endpoint (§9.2) as composing `SquadPlayer` (§6.3), `PlayerSeasonStatistics` (§6.6), and current-Gameweek `GameweekRoster`/`RosterPlayer` membership (§6.5), and reusing the identical position/search/sort query-parameter implementation introduced for the Draft Player Pool (§6.4) rather than a second implementation of the same filtering behavior.

## Version 1.9

Incorporates BRD v1.11's formalization of the Administrative Audit Log Viewer's filtering and detail-expansion behavior (BR-321–BR-322), identified while reviewing an illustrative HTML mock-up of F-011.1: documents the audit read endpoint's `actionType`/`from`/`to`/`fantasyTeamId` query parameters (§9.2), including the "League Settings" pseudo-value for `AdministrativeAction` rows whose `TargetEntityType` isn't a `FantasyTeam` (§6.8); and clarifies that per-entry detail expansion requires no new endpoint, since `BeforeState`/`AfterState` are already returned with every row.

## Version 1.10

Incorporates BRD v1.12's formalization of Draft pick timeout/makeup-pick visibility (BR-323–BR-325), identified while reviewing an illustrative HTML mock-up of a Draft Timeouts & Makeup Picks screen: adds the `DraftPickSkipped` domain event (§6.4, §7), raised on every skip — first-time or re-queue, distinguished by an `IsRequeue` flag — so a Reporting-context projection can drive the skip/re-queue history a Draft UX screen needs; and clarifies that the pending-queue display (BR-324) requires no event at all, since it reads `Draft.PendingMakeupPicks` directly.

## Version 1.11

Incorporates BRD v1.13's resolution of the "Historical Username Display" open decision (BR-326): adds the `UsernameHistory` entity (§6.1) — an append-only per-User username timeline maintained by the existing `UsernameChanged` event — and its partial unique index (§8.1). Historical screens resolve a display username by looking up the `UsernameHistory` row effective at the record's timestamp rather than joining directly to `User.Username`, so no existing historical table needs a username column added to it, and no backfill is required beyond seeding one open-ended row per existing User.

## Version 1.12

Incorporates BRD v1.14's System Administrator visibility rules (BR-327–BR-328), identified while reviewing an illustrative HTML mock-up of a Security & Abuse Protection screen: adds the `SecurityEvent` read-model (§6.8), a platform-level (not League-scoped) log written by the rate-limiting middleware on every enforced block, satisfying BR-170's existing logging requirement as a queryable table rather than only an application-log line; documents the System-Administrator-only `/api/v1/admin/security/*` endpoint group (§9.2); and clarifies that CSRF-posture visibility (BR-328) is a configuration read, not a new persisted entity.

## Version 1.13

Incorporates BRD v1.15's new EPL League Table and Gameweek Fixtures pages (BR-329–BR-335) — the first requirement to expose the Player & EPL Data context's reference data (`Player`, `Club`, `Fixture`, `Gameweek`; module map, §5) directly as its own user-facing page, rather than only as an input to Fantasy scoring/scheduling. Adds the `ClubStanding` read-model (§6.10), recomputed from synced `Fixture` results the same way `LeagueStanding` is recomputed (§6.7), with no Administrator-override path since it reflects the real EPL, not Fantasy scoring; documents the platform-level (not League-scoped) `/api/v1/epl/*` endpoint group (§9.2); and notes the Gameweek Fixtures page needs no new entity, since it queries existing `Fixture` rows already keyed by `GameweekId`.

## Version 1.14

Incorporates BRD v1.16's Gameweek EPL schedule and per-player opponent display on the Weekly Roster screen (BR-336–BR-337), reusing rather than duplicating the `Fixture` data introduced for F-004.6 (§6.10, Version 1.13): documents both as read-time projections in the Roster Management context (§6.5) — the Gameweek's fixture list is the same read already exposed at `/api/v1/epl/gameweeks/{gameweekId}/fixtures`, and the per-player opponent is a cross-context join (`Player.CurrentClubId` against that Gameweek's `Fixture` rows) computed when the roster screen loads, with no new persisted entity or domain event. Documents the roster-screen read endpoint in the API table (§9.2), which had not previously been enumerated.

## Version 1.15

Incorporates BRD v1.17's League-scoping of notification preferences (BR-338–BR-339): re-keys `NotificationPreference` (§6.9) from `UserId` to `LeagueMembershipId`, so a User's preferences for Gameweek Reminder/Weekly Score/Weekly Standings are held once per League Membership rather than once globally; adds `LeagueMembershipId` to `NotificationRequest` so the outbox worker checks the correct League's preference row (BR-226); and removes `UserProfile.NotificationPreferencesSummary` (§6.1), which assumed a single global value that no longer exists — a screen showing all of a User's preferences now reads across that User's `LeagueMembership`-keyed rows directly. No new bounded context or endpoint group: this is a re-keying of an existing entity, not a new capability.
