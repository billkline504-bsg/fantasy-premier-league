...
## Feature Behavior Specifications — Phase 2 (Draft & Squad) — Version 1.3

**Document Status:** Baseline Feature Specifications
**Version:** 1.3
**Inputs:**
- Business Requirements Document v1.12 — authoritative business rules (BR-001…BR-325)
- Architecture and Domain Model v1.10 — aggregates, invariants, module map
- Epic and Feature Backlog v1.4 — feature list, dependencies, build sequence
...
9. Given a FantasyTeam's pick times out with no Administrator extension, when the ADR-012 sweep skips it, then the skip is visibly indicated to Draft participants, distinct from a completed selection (BR-323) — driven by the `DraftPickSkipped` domain event (Architecture v1.10 §6.4).
10. Given the final regularly scheduled round has completed, when one or more picks were skipped during the Draft, then the current pending makeup-pick queue is displayed, showing which FantasyTeams are waiting and the order they will pick in (BR-324) — read directly from `Draft.PendingMakeupPicks` (Architecture v1.10 §6.4).
11. Given a makeup pick is itself skipped and re-queued, when this occurs, then the re-queue is visibly distinguished from an initial skip (BR-325) — the same `DraftPickSkipped` event, but with `IsRequeue = true` (Architecture v1.10 §6.4).
...
**Technical tasks:** read-model/query endpoints for draft order, current turn, pick history, and player-ownership status, extended with `position`, `search`, and `sort` query parameters (Architecture v1.7 §6.4) and season-to-date statistics sourced from `PlayerSeasonStatistics` (Architecture v1.7 §6.6); a skip/re-queue history projection consuming `DraftPickSkipped` (Architecture v1.10 §6.4/§7) for acceptance criteria 9 and 11; the pending-queue display (criterion 10) needs no new projection, since it reads `Draft.PendingMakeupPicks` directly.
...
**Added in v1.3 (not an open item raised while originally writing this phase):** reviewing an illustrative HTML mock-up of a Draft Timeouts & Makeup Picks screen surfaced three more gaps — F-005.4/BR-282 governs *that* a pick gets skipped and re-queued, but nothing required this to be *visible* to participants. Closed as BR-323–BR-325 (BRD v1.12), folded into F-005.5 (acceptance criteria 9–11 above) rather than F-005.4, since F-005.4 is backend behavior only and F-005.5 already owns every other Draft display requirement. Supporting change: the new `DraftPickSkipped` domain event (Architecture v1.10 §6.4/§7).
