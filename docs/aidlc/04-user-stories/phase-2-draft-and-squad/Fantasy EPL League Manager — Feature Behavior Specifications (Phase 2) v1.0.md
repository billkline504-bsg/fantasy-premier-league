...
## Feature Behavior Specifications — Phase 2 (Draft & Squad) — Version 1.0
...
4. Given a League with fewer than two FantasyTeams for the Season, when Draft start is attempted, then it is rejected — a draft requires at least two competing teams. *(Inferred requirement, not an explicit BR; flag for confirmation — see Open Items.)*
...
**Technical tasks:** `PendingMakeupPicks` queue on the `Draft` aggregate; a detection mechanism for expired deadlines — **whether this is a scheduled background sweep or a lazy check performed on the next relevant read/write is an implementation detail not yet decided; flag for confirmation (see Open Items).**
...
**Technical tasks:** read-model/query endpoints for draft order, current turn, pick history, and player-ownership status.
...
3. Given a FantasyTeam owner does not immediately use an available opportunity, when time passes, then the opportunity remains available until used — **no forced use-it-or-lose-it deadline is specified in the BRD; flag for confirmation (see Open Items).**
...
Acceptance criteria:
1. Given a fantasy-owned player is detected as having transferred out of the EPL (via F-004.4), when the Administrator reviews the eligibility queue, then this candidate appears pre-flagged from official data; the Administrator confirms it to trigger F-006.4's eligibility grant (BR-066).
2. Given league discussion (outside the application) reaches consensus that a fantasy-owned player has a season-ending injury, when the Administrator records that determination, then the player becomes replacement-eligible and the determination is captured as an `AdministrativeAction`, including who made it, when, and any recorded rationale (BR-067, BR-068, BR-149).
3. Given an eligibility determination has been made, when it is viewed later (by the Administrator or in an audit review), then it is fully auditable (BR-149, BR-295-style pattern).
4. **Open question raised while specifying this feature:** BR-066 (EPL exit) reads as automatic ("any fantasy-owned player who transfers out of the EPL becomes eligible"), while BR-065's general principle says eligibility requires League Administrator determination. AC-1 above assumes EPL-exit still needs a lightweight Administrator *confirmation* click (not full judgment, just triggering the grant) rather than being fully automatic with no admin step at all. This should be confirmed before implementation — see Open Items.
...
1. **F-005.1 (Draft Setup):** whether a minimum of two FantasyTeams is required to start an Initial Draft. Assumed **yes** as an inferred common-sense default — not an explicit BR. Confirm or override.
2. **F-005.4 (Draft Timeout):** whether expired-deadline detection is a scheduled background sweep or a lazy check performed on the next relevant read/write. Both are viable; this is an implementation-architecture decision, not a business rule, but should be made explicitly before F-005.4 is built.
3. **F-006.4 (Replacement Drafting):** whether a replacement-selection opportunity ever expires if unused (a "use it or lose it" deadline). Assumed **no expiration** as the conservative default, consistent with BR-064's "does not need to replace... before making another secondary-draft selection" language. Confirm or override.
4. **F-011.2 (Eligibility Determination):** whether an EPL-exit-triggered replacement eligibility grant genuinely requires an Administrator confirmation step, or whether BR-066 intends it to be fully automatic with no admin action at all. This is a real tension between BR-065 (general principle: admin determines) and BR-066 (specific case: reads as automatic) that should be resolved in the BRD rather than left to implementer interpretation.
