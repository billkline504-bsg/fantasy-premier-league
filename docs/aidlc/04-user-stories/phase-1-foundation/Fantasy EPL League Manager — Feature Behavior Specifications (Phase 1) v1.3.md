...
## Feature Behavior Specifications — Phase 1 (Foundation) — Version 1.3

**Document Status:** Baseline Feature Specifications
**Version:** 1.3
**Inputs:**
- Business Requirements Document v1.15 — authoritative business rules (BR-001…BR-335)
- Architecture and Domain Model v1.13 — aggregates, invariants, module map
- Epic and Feature Backlog v1.7 — feature list, dependencies, build sequence

**Scope:** This document covers **build-sequence positions 1–20** from the Backlog's §5 — every P0 feature in Phase 1 (Foundation): all of EPIC-001 (Identity & Authentication), EPIC-002 (User Profile & FantasyTeam), EPIC-003's P0 features (League & Season Management, excluding P1 League Messaging), EPIC-004 (EPL/FPL Data), and F-010.3 (Season Goal Prediction submission — pulled forward from EPIC-010 per the Backlog's explicit note that its submission path must exist at Season start). Also includes F-004.6 (EPL League Table & Fixtures Display — added in this version, Backlog position 57), a P1 feature grouped here with the rest of EPIC-004 rather than deferred to a later phase, since it depends only on features already specified in this document. Phases 2–4 (Draft, Weekly Gameplay, Operations) are follow-on documents, written in the same format once this phase is implementation-ready.
...
3. Given a username used only by a *retired* user, when I attempt to register with it, then registration succeeds — the uniqueness check only considers active users, so a retired user's username is immediately available to any other individual (BR-004, BR-298).
...
3. Given a user retires and a different individual later registers using that now-available username, when both users' histories are queried, then each resolves correctly and independently by `UserId` — the two individuals' historical records are never conflated merely because they shared a username string at different times (BR-298, BR-012).
4. Given a completed historical Season referencing my old username in a snapshot/report, when I later change my username, then that historical snapshot continues to display the username I held **at the time** the record was created — not my new one (BR-326, BRD v1.13; resolves what was previously the open BRD §54 "Historical Username Display" decision).
5. Given a username change, when it is applied, then the currently-open `UsernameHistory` row for my `UserId` is closed (`EffectiveTo = now`) and a new row is opened (`EffectiveFrom = now`) in the same transaction as the `UsernameChanged` event (BR-326, Architecture v1.11 §6.1) — this is what makes AC-4 possible without touching any historical table.
...
**Technical tasks:** username-change endpoint, re-validation against the active-user unique index, no `UserId` mutation path exists in the domain model (defense against accidental identity change); the same transaction closes/opens the `UsernameHistory` row (Architecture v1.11 §6.1).
...
4. Given a retired user's username, when a *different* prospective user attempts to register with that same username, then registration succeeds — retirement immediately frees the username for reuse (BR-298; see F-001.1 AC-3).

**User Story 2:** As a System Administrator, I want to reactivate a retired User account when justified, so that a user who retired in error (or wishes to return in a supported way) can regain access without a public self-service path that could be misused.

Acceptance criteria:
5. Given a `Status = Retired` User and a caller with `IsSystemAdministrator = true`, when reactivation is requested, then `Status` transitions back to `Active` and `RetiredAt` is cleared (BR-301).
6. Given a caller who is *not* a System Administrator (including a League Administrator, and including the affected user themselves), when they attempt to reactivate a retired account, then the request is rejected — no self-service reactivation path exists in this release (BR-301).
7. Given a retired user's original username has since been taken by a different active user (per AC-4), when a System Administrator reactivates the account, then the operation requires assigning a new username as part of reactivation — it cannot silently reuse a username already claimed by someone else (BR-301).

**Security considerations:** retirement is a status transition, never a physical delete, so no code path can hard-delete a `User` row; reactivation is gated on `IsSystemAdministrator`, a flag that is never settable through any public/self-service API (Architecture v1.3 ADR-007) — it can only be set out-of-band by an operator.

**Technical tasks:** status-transition method on `User` aggregate guarded so no code path can hard-delete a `User` row; login-guard check on `Status`; System-Administrator-only reactivation endpoint with the username-conflict check in AC-7; `UserReactivated` domain event.
...
3. Given a `FantasyTeam` that did not submit a prediction before Season start, when that FantasyTeam's owner attempts to submit their **first** Gameweek roster of the Season, then the application prompts them to submit the Season Goal Prediction and does not accept the roster submission until they do (BR-299). Once submitted this way, `LockedAt` is set to the submission time and the prediction is treated identically to one submitted at Season start for all tie-break purposes. This precondition is also captured on the Phase 3 roster-submission feature (F-007.1) when that spec is written — it is a cross-feature dependency, not solely owned by this feature.
...
**Technical tasks:** `SeasonGoalPrediction` aggregate; lock-on-season-start guard; the missing-prediction precondition check on first `GameweekRoster` submission is implemented as an application-service-level cross-context check (Architecture v1.3 §6.7), coordinated with F-007.1 when Phase 3 is built.
...
## F-004.6 — EPL League Table & Fixtures Display
**Backlog position:** 57 (added in Backlog v1.7 — see that document's build-sequence note) · **Depends on:** F-004.1 (club reference data), F-004.2 (fixture/gameweek data)

**User Story:** As a user, I want to see the actual, real-world EPL league table and each Gameweek's fixtures and results, separate from any Fantasy League's own standings or schedule, so I can follow the real competition my fantasy squad is drawn from.

Acceptance criteria:
1. Given the current EPL season's synced fixture results, when a user views the EPL League Page, then the current official league table is displayed with Position, Club, Played, Won, Drawn, Lost, Goals For, Goals Against, Goal Difference, and Points, ordered per official EPL rules (BR-329, BR-330).
2. Given a past EPL season for which historical data has been retained, when a user selects that season on the EPL League Page, then its final table is displayed the same way (BR-331).
3. Given official Gameweeks (BR-092), when a user views the Gameweek Fixtures Page, then fixtures are grouped by Gameweek and the user can browse past, current, and future Gameweeks (BR-332).
4. Given a specific fixture, when it is displayed, then the two competing clubs and scheduled kickoff date/time are always shown; a completed fixture additionally shows its final score; an in-progress fixture shows the live score where available from official data; a future fixture shows no score (BR-333).
5. Given this page reflects the real EPL rather than any Fantasy League, when any user (regardless of League membership, or lack thereof) views it, then access is granted — it is not gated by League membership, and no League Administrator score override (BR-139–BR-145) ever applies to it (BR-334, BR-335).

**Security considerations:** none beyond the shared external-source risk (F-004.1) — this is a read-only, platform-level page with no League-scoped or object-level authorization check, unlike every other competition-facing screen in this backlog.

**Technical tasks:** `ClubStanding` read-model, recomputed from synced `Fixture` results the same way `LeagueStanding` is recomputed (Architecture v1.13 §6.10); `GET /api/v1/epl/seasons/{eplSeasonId}/table` and `GET /api/v1/epl/gameweeks/{gameweekId}/fixtures` endpoints (Architecture v1.13 §9.2); no new entity needed for fixtures display, since it queries existing `Fixture` rows already keyed by `GameweekId`.

---

# Open Items Raised While Writing This Phase

Three items surfaced during specification (v1.0) and were resolved in BRD v1.6 (§63) and Architecture v1.3:

1. **F-001.3 (Username Management):** a retired user's username becomes available for reuse by any user, including a different individual (BR-298). Resolved in this revision — see F-001.1 AC-3, F-001.3 AC-3/AC-4.
2. **F-010.3 (Season Goal Prediction):** a FantasyTeam that never submits a prediction before Season start is prompted and blocked from submitting its first Gameweek roster until it does (BR-299). Resolved in this revision.
3. **F-001.4 (User Retirement):** account reactivation is supported, but only by a new platform-level System Administrator role — no self-service reactivation (BR-300, BR-301). Resolved in this revision — see F-001.4 User Story 2.

No open items remain from the v1.0 pass. Any new items raised while writing Phase 2 (Draft & Squad) specs will be tracked in that document.

**Added in v1.2 (not an open item raised while originally writing this phase):** F-001.3 AC-4 previously deferred entirely to the open BRD §54 "Historical Username Display" decision. That decision was resolved (BR-326, BRD v1.13) after comparing both options against a concrete example in an illustrative HTML mock-up — historical records now display the username active at the time of the event, maintained via the new `UsernameHistory` entity (Architecture v1.11 §6.1). AC-4 and the technical tasks above are updated accordingly.

**Added in v1.3 (not an open item raised while originally writing this phase):** reviewing the BRD as a whole (not a mock-up) surfaced that no feature anywhere covered displaying the actual, real-world EPL league table or EPL fixtures/results — the existing Standings (F-010.4) and Schedule (F-009.4) features only ever covered Fantasy League standings and FantasyTeam-vs-FantasyTeam matchups. Closed as BR-329–BR-335 (BRD v1.15) and a brand-new feature, F-004.6, grouped into EPIC-004 since it depends only on that epic's existing sync features — the same kind of previously-unowned-capability gap as F-007.5 (Squad View, Phase 3 v1.2) and F-011.3 (Security & Abuse Monitoring, Phase 4 v1.4).
