...
## Epic and Feature Backlog — Version 1.10

**Document Status:** Baseline Backlog
**Version:** 1.10
**Inputs:**
- Business Requirements Document v1.5 (`Fantasy EPL League Manager — Business Requirements Document v1.5.md`) — authoritative business rules (BR-001…BR-297); F-005.5's BR references were refreshed against BRD v1.9 (BR-310–BR-313) and again against BRD v1.12 (BR-323–BR-325), a new F-007.5 Squad View feature was added tracing to BRD v1.10 (BR-209, BR-314–BR-320), F-011.1's BR references were corrected and extended against BRD v1.11 (BR-321–BR-322, replacing the mis-cited BR-239–BR-242), F-001.3/F-013.2's BR references were extended against BRD v1.13 (BR-326, resolving the previously-open "Historical Username Display" decision), a new F-011.3 Security & Abuse Monitoring feature was added tracing to BRD v1.14 (BR-327–BR-328), a new F-004.6 EPL League Table & Fixtures Display feature was added tracing to BRD v1.15 (BR-329–BR-335), F-007.1's BR references and dependencies were extended against BRD v1.16 (BR-336–BR-337, Gameweek EPL schedule and per-player opponent display on the Weekly Roster screen), F-012.1's BR references and dependencies were extended against BRD v1.17 (BR-338–BR-339, per-League Membership notification preference scoping), and F-012.2/F-012.3/F-012.4's BR references were extended to cite BR-338 (the League-scoped preference check their own Feature Behavior Specifications already described applies to them too), without otherwise changing this document's baseline
...
| F-001.3 | Username management (change, uniqueness, historical display resolution) | BR-003, BR-004, BR-265, BR-266, BR-270, BR-326 | F-001.1 | P0 | S |
...
| F-004.6 | EPL league table & fixtures display (real-world, read-only; distinct from Fantasy League Standings/Schedule) | BR-329–BR-335 | F-004.1, F-004.2 | P1 | M |
...
| F-005.5 | Draft UX (order, current turn, history, ownership, player-pool filter/search/sort/stats, skip/makeup-queue/re-queue visibility) | BR-204, BR-205, BR-207, BR-208, BR-310–BR-313, BR-323–BR-325 | F-005.2 | P0 | M |
...
| ID | Feature | Traces to | Depends On | Priority | Size |
|---|---|---|---|---|---|
| F-007.1 | Weekly roster submission (including Gameweek EPL schedule and per-player opponent display) | BR-037, BR-041, BR-196, BR-211, BR-279, BR-291, BR-336, BR-337 | F-005.2 (squad must exist), F-004.2 (fixture data, for BR-336–BR-337) | P0 | L |
...
| F-007.5 | Squad View (full squad, position filter/search/sort, acquisition type, season stats, replacement-eligibility) | BR-209, BR-314–BR-320 | F-005.2 (squad must exist) | P0 | M |
...
| ID | Feature | Traces to | Depends On | Priority | Size |
|---|---|---|---|---|---|
| F-011.1 | Administrative audit log viewer (filter by action/date/team, per-entry detail expansion) | BR-177–BR-182, BR-321–BR-322 | F-007.4, F-008.5 | P1 | M |
| F-011.3 | Security & abuse monitoring (System Administrator: rate-limit config/events, CSRF posture) | BR-168, BR-169, BR-170, BR-327, BR-328 | F-001.1 | P1 | S |
...
| ID | Feature | Traces to | Depends On | Priority | Size |
|---|---|---|---|---|---|
| F-012.1 | Notification preferences management (per-League Membership, managed within the Profile screen) | BR-150, BR-151, BR-155, BR-226, BR-338, BR-339 | F-001.1, F-003.3 (LeagueMembership must exist, BR-338), F-002.4 (per-League Profile section to hold these settings, BR-339) | P1 | M |
| F-012.2 | Gameweek reminder notifications | BR-152, BR-338 | F-012.1, F-007.3 | P1 | M |
| F-012.3 | Weekly score/standings notifications | BR-153, BR-154, BR-338 | F-012.1, F-008, F-010 | P1 | M |
| F-012.4 | Notification delivery infrastructure (async outbox, retry) | BR-224, BR-225, BR-338 | F-012.1 | P1 | L |
...
| F-013.2 | Retired-user historical display | BR-014, BR-175, BR-272, BR-326 | F-013.1 | P2 | S |
...
**Added in v1.2 (discovered after this build sequence was finalized — appended rather than renumbering positions 26–54 above, all of which are already cited by exact number in the published Phase 2–4 feature specifications):**

55. F-007.5 Squad View *(dependency-wise this slots immediately after F-005.2, alongside F-005.5 — build it whenever convenient between positions 25 and 26 above; it is listed at 55 only because it was identified later, not because of a real ordering constraint)*

**Added in v1.6 (same reasoning as v1.2 — appended rather than renumbering):**

56. F-011.3 Security & Abuse Monitoring *(depends only on F-001.1 — authentication and the rate-limiting/CSRF infrastructure it relies on must exist; could reasonably be built any time after position 1, listed at 56 only because it was identified later)*

**Added in v1.7 (same reasoning as v1.2/v1.6 — appended rather than renumbering):**

57. F-004.6 EPL League Table & Fixtures Display *(depends only on F-004.1/F-004.2, both already at positions 16–17 — could reasonably be built any time after position 17, listed at 57 only because it was identified later)*

**Added in v1.8:** no new position — F-007.1 (position 26) gains an additional dependency on F-004.2 (position 17), which was already required earlier in the sequence than F-007.1 for an unrelated reason (kickoff times, per F-007.3's existing note), so this does not change the build order.

**Added in v1.9:** no new position — F-012.1 (position 50) gains additional dependencies on F-003.3 (position 4) and F-002.4 (position 11), both already far earlier in the sequence, so this does not change the build order. F-002.4 was already an implicit prerequisite for showing per-League settings at all; BR-338–BR-339 (preferences are per-League Membership, managed within the Profile screen) simply make that dependency explicit.

**Added in v1.10:** no new position and no new dependency — F-012.2 (position 52), F-012.3 (position 53), and F-012.4 (position 51) already depend on F-012.1, which now owns the League-scoped `NotificationPreference` model (BR-338). Their trace lists gain BR-338 purely for citation accuracy: each of these three features' own Feature Behavior Specifications (Phase 4) already required checking the correct League's preference row, but the Backlog table hadn't cited the rule that requirement comes from.
