...
## Business Requirements Document — Version 1.17
...
## BR-338 — Notification Preferences Are League-Independent

Because Gameweek Reminder (BR-152), Weekly Score (BR-153), and Weekly Standings (BR-154) notifications are each generated in the context of a specific League a User participates in, notification preferences for these event types shall be configurable independently for each League Membership (BR-017) the User holds, not as a single setting shared across all of a User's Leagues. Enabling or disabling a given event type/channel combination in one League shall have no effect on that same combination in any other League the User participates in.

## BR-339 — Per-League Notification Preferences Location

Each League's notification preferences shall be managed within that League's own section of the Profile screen (BR-202), alongside that League's icon settings, rather than via a separate standalone Notifications screen.
...
## BR-327 — Rate Limit Configuration & Event Visibility

A System Administrator shall be able to view the platform's currently configured rate limits (endpoint, limit, window, and scope) and a chronological log of recent rate-limit enforcement events.

## BR-328 — CSRF Protection Status Visibility

A System Administrator shall be able to view the platform's current CSRF protection posture, including whether cookie-based authentication flows are in use and, if so, confirmation that anti-CSRF protections are active for them.
...
## BR-336 — Gameweek EPL Schedule Display

The Weekly Roster selection screen shall display the current Gameweek's EPL fixtures (BR-332), so a User can see the full round of official matches while building their roster.

## BR-337 — Player Gameweek Opponent Display

For each player shown on the Weekly Roster selection screen — whether currently placed in the roster or remaining in the squad — the application shall display that player's EPL club's opponent for the current Gameweek and whether the player's club is home or away, derived from the Gameweek Fixtures data (BR-332).
...
| DEC-085 | A System Administrator can view the platform's current rate-limit configuration and recent rate-limit enforcement events, and can view whether cookie-based auth flows (and their CSRF protections) are currently active |
| DEC-086 | A dedicated EPL League Page (current and historical official league tables) and Gameweek Fixtures Page (past, current, and future EPL fixtures/results) are added, sourced exclusively from official external data and visible platform-wide independent of Fantasy League membership |
| DEC-087 | The Weekly Roster selection screen additionally displays the current Gameweek's full EPL fixture list, and each squad player's individual EPL opponent (and home/away status) for that Gameweek, to help Users decide their roster |
| DEC-088 | Notification preferences (Gameweek Reminder, Weekly Score, Weekly Standings) are configured independently per League Membership rather than as a single global setting, and are managed within each League's own section of the Profile screen rather than a standalone Notifications screen |
...
Formalizes System Administrator visibility into two existing security controls, adding BR-327–BR-328 (see §71): rate-limit configuration and recent enforcement events (BR-169), and CSRF protection posture (BR-168). Neither BR-168 nor BR-169 required this visibility to exist anywhere — they govern the controls themselves, not who can observe them — identified while reviewing an illustrative HTML mock-up of a System Administrator Security & Abuse Protection screen. A new Feature (F-011.3) and a lightweight `SecurityEvent` log (Architecture v1.12 §6.8) support this.

## Version 1.15

Adds a dedicated real-world EPL League Page and Gameweek Fixtures Page, formalized as BR-329–BR-335 (see §72). This closes a gap identified after the fact rather than during a mock-up review: Section 39 (Standings) and Section 40 (Head-to-Head Schedule) describe only Fantasy League standings and FantasyTeam-vs-FantasyTeam matchups — no BR previously established a page presenting the actual Premier League table or actual EPL fixtures/results, even though the underlying external data (player, fixture, and gameweek data — BR-228–BR-230) has been part of External EPL/FPL Integration (Section 43) since Version 1.0. The new page(s):

- Display the current-season official EPL table and any retained past season's final EPL table
- Display EPL fixtures grouped by official Gameweek, browsable across past, current, and future Gameweeks, with results shown for completed fixtures and live scores where available for in-progress ones
- Are sourced exclusively from official external data, are not subject to Fantasy League Administrator score overrides, and are visible to any authenticated user regardless of League membership, since they reflect the real-world competition rather than any single Fantasy League

## Version 1.16

Adds BR-336–BR-337 (see §73), extending the Weekly Roster User Experience (Section 38) with two pieces of real-world EPL context that were not previously surfaced anywhere on the roster selection screen: the current Gameweek's full EPL fixture list, and, for each squad player individually, that player's EPL club's opponent (and home/away status) for the Gameweek. Both derive from the Gameweek Fixtures data already established for the EPL League Page (BR-332, BRD v1.15) — this version reuses that data on a second screen rather than introducing a new data source.

## Version 1.17

Adds BR-338–BR-339 (see §74), formalizing a scope correction identified while reviewing an illustrative HTML mock-up of the Profile screen: Section 26's existing notification rules (BR-150–BR-155) never established whether a User's notification preferences were global or per-League, and the mock-up had already (correctly) built them as independent per-League settings, nested within each League's own section of the Profile screen — since Gameweek Reminder, Weekly Score, and Weekly Standings are all inherently League-specific events. BR-338 formalizes the League-independent scope; BR-339 formalizes the screen location.

---

# 60. Final Baseline

BRD v1.17 represents the current functional baseline for AIDLC generation.
...
# 71. Version 1.14 — Resolved Items from Security & Abuse Protection Mock-Up Review

Section 28 (Security) has required CSRF protection (BR-168) and rate limiting (BR-169) since early in this document's history, and Section 46 (Observability) has required security-relevant events to be logged (BR-239–BR-242) since the same baseline. Reviewing an illustrative HTML mock-up of a System Administrator "Security & Abuse Protection" screen surfaced that neither rule ever said anyone could actually *see* the resulting configuration or events — BR-168/BR-169 govern the controls themselves, and BR-239–BR-242 (as already established in Version 1.11, §68) are general logging/observability rules, not a viewing capability for a specific administrator.
...
Both rules are System-Administrator-scoped (BR-300), not League-Administrator-scoped — this is platform-level visibility that sits above every League, distinct from the per-League Audit Log (F-011.1, BR-321–BR-322).

---

# 72. Version 1.15 — EPL League Table and Fixtures

This section formalizes a gap identified on review of the BRD as a whole, not from a mock-up review. Section 39 (Standings) and Section 40 (Head-to-Head Schedule) describe only Fantasy League standings and matchups between FantasyTeams. No prior BR established a page presenting the actual, real-world EPL league table or actual EPL fixtures/results — distinct from any Fantasy League's standings or H2H schedule — even though the external data needed to build one (player data, fixture data, gameweek data) has been part of External EPL/FPL Integration (Section 43, BR-227–BR-234) since Version 1.0.

## BR-329 — EPL League Page

The application shall provide a page displaying the official current-season EPL league table, reflecting real-world club standings independent of any Fantasy League's standings (BR-215–BR-217).

## BR-330 — EPL Table Content

The EPL table shall display, per club: Position, Club, Played, Won, Drawn, Lost, Goals For, Goals Against, Goal Difference, and Points, ordered according to official EPL rules.

## BR-331 — Historical EPL Tables

The application shall provide access to the final EPL table for any past EPL season for which historical data has been retained.

## BR-332 — Gameweek Fixtures Page

The application shall provide a page displaying EPL fixtures grouped by official Gameweek (BR-092), browsable across past, current, and future Gameweeks.

## BR-333 — Fixture Display

Each fixture shall display the two competing clubs and the scheduled kickoff date/time. A completed fixture shall additionally display its final score; a fixture in progress shall display the live score where available from official data; a future fixture shall display no score.

## BR-334 — EPL Data Authority

The EPL League Page and Gameweek Fixtures Page shall be sourced exclusively from official external EPL/FPL data (BR-227–BR-234) and are not subject to Fantasy League Administrator score overrides (BR-139–BR-145), which apply only to Fantasy scoring.

## BR-335 — Platform-Level Visibility

The EPL League Page and Gameweek Fixtures Page shall be visible to any authenticated user regardless of League membership, since they present the real-world competition rather than any single Fantasy League's data.

---

# 73. Version 1.16 — Gameweek EPL Schedule and Player Opponent Display

This section formalizes a gap identified after Section 39/40 (the Weekly Roster User Experience, BR-209–BR-214) was compared against the newly added EPL League Page and Gameweek Fixtures Page (BR-329–BR-335, Section 72): a User deciding their weekly roster had no way to see, on that same screen, which real-world EPL opponent each of their squad's players actually faces that Gameweek — information directly useful to the roster decision itself (BR-210), since a player facing a stronger or weaker opponent is a normal part of how a User weighs their selection.
...
Both rules reuse the Gameweek Fixtures data introduced for the EPL League Page/Gameweek Fixtures Page (BR-332, BRD v1.15) rather than requiring a second data source: BR-336 surfaces that same fixture list on the Weekly Roster screen, and BR-337 is a per-player derivation from it — resolving, for a given player, the fixture (if any) in which that player's `Player.CurrentClubId` appears for the current Gameweek, and displaying the other club in that fixture as the opponent.

---

# 74. Version 1.17 — Notification Preferences Are League-Scoped

Section 26 (Notifications) has required user-configurable notification preferences (BR-150, BR-155) since early in this document's history, but never stated whether those preferences are held once per User or once per League a User belongs to. Reviewing an illustrative HTML mock-up of the Profile screen surfaced that this ambiguity was already resolved in practice: the mock-up nests a "Notifications for This League" block inside each League's own section of the Profile screen, with fully independent toggles per League — a User in two Leagues can, for example, enable Weekly Score emails in one and disable them in the other. That design choice is correct (Weekly Score and Weekly Standings are themselves League-specific concepts — a User's score and standing differ by League — and even Gameweek Reminder is tied to a specific League's roster-lock deadline), but nothing in the BRD had said so.
...
Together these rules mean `NotificationPreference` (Architecture §6.9) is keyed by `LeagueMembershipId`, not `UserId` — a User's notification preferences are exactly as numerous as their League Memberships, each independently configurable, and there is no longer a single global on/off switch for these three event types. BR-151 (channel selection) and BR-155 (independence by event type and channel) continue to apply *within* each League's preference set; BR-338 adds the additional per-League dimension neither rule previously established.